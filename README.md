...
wt merge auth                              # 自动 merge 到 main（rebase + squash + 清理）
...
| `wt merge <name\|index> [--agent]` | 自动 merge 到 main（rebase + squash + 清理）|
| `wt reset <name\|index>` | 重置到 pending（备份代码）|
| `wt new [name]` | 创建 scratch 环境 |
| `wt delete <name>` | 删除 scratch 环境 |
...
wt status --action complete --task ui  # 标记完成
...
- **review** 标记任务待审核，关闭 tmux 窗口
- **resume** 从 Review 恢复到 Running，继续开发
- **merge** 执行 rebase + squash merge + commit，然后自动清理资源
